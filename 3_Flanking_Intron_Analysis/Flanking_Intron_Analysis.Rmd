...
#Create list of all introns flanking circRNAs
flankingIntrons <- rbind(upIntrons, dwnIntrons)

```
##Extract all introns in circRNA-forming genes/transcripts:
```{r}
#load biomart:
ensemblGRCh37 <- useEnsembl(biomart = 'genes', dataset = 'hsapiens_gene_ensembl', version = "GRCh37")

hostTranscripts <- unique(annotatedCirc$transcript) #list of circ-forming genes(transcript=longest transcript containing the/all circularized exon/s)

#Extract info on transcript start and stop sites and exon start and stop sites for each transcript
hostInfo <- getBM(attributes = c('ensembl_transcript_id', 'chromosome_name', 'strand', 'transcript_start','transcript_end', 'ensembl_exon_id', 'exon_chrom_start', 'exon_chrom_end'),
                 filters = 'ensembl_transcript_id',
                 values = hostTranscripts,
                 mart = ensemblGRCh37)

#Create bed files for transcript and exon info:
hostInfo$score <- rep(".", length(hostInfo$ensembl_transcript_id)) #make score column
hostInfo$strand <- gsub("-1", "-", hostInfo$strand) #convert to standard strand expression
hostInfo$strand <- gsub("1", "+", hostInfo$strand) #convert to standard strand expression

#Subset exon position info to bed format:
exonBed <- unique(hostInfo[,c(2,7,8,6,9,3)])
write.table(exonBed, file = "hostExons.bed", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE) #save
#Subset transcript position info to bed format:
transcriptBed <- unique(hostInfo[,c(2,4,5,1,9,3)])
write.table(transcriptBed, file = "hostTranscripts.bed", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE) #save

#Create bed for all flanking introns:
flankingIntrons$score <- rep(".", length(flankingIntrons$id)) #make score column
flankingIntrons$chrom <- gsub("chr","", flankingIntrons$chrom)
flankIntBed <- flankingIntrons[,c(3,5,6,2,7,4)] #Subset intron position info to bed format:
write.table(flankIntBed, "flankingIntrons.bed", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE) #save


```

```{bash}
bedtools sort -i hostExons.bed > hostExons.sort.bed #sort bed file
bedtools merge -i hostExons.sort.bed -c 4,5,6 -o collapse,distinct,distinct -s > hostExonsMerge.bed #To remove potential redundancy is removed by using merge to collapse overlapping and book-ended exons in stand-specific manner (-s)  
bedtools sort -i hostTranscripts.bed > hostTranscripts.sort.bed #sort bed file
bedtools subtract -a hostTranscripts.sort.bed -b hostExonsMerge.bed -s > hostIntrons.bed #Intron positions are found by subtracting (merged) exon position from transcript regions.
bedtools sort -i flankingIntrons.bed > flankingIntrons.sort.bed #sort bed file
bedtools subtract -a hostIntrons.bed -b flankingIntrons.sort.bed -s -A > nonFlnkIntrons.bed
#Subtract introns which flank circRNAs from all host introns -A options removes all features in -a with any overlap in -b.

```

##Analyse intron length flanking and non flanking
```{r}
hostIntrons <- read.delim("./hostIntrons.bed", header = FALSE)
nonFlnkIntrons <- read.delim("./nonFlnkIntrons.bed", header = FALSE)


nonFlnkIntrons$intLength <- nonFlnkIntrons$V3 - nonFlnkIntrons$V2
nonFlnkIntrons$group <- rep("nonFlnkInt", length(nonFlnkIntrons$V1))
nonFlnkIntLen <- nonFlnkIntrons[,c(8,7)]

hostIntrons$intLength <- hostIntrons$V3 - hostIntrons$V2
hostIntrons$group <- rep("hostInt", length(hostIntrons$V1))
hostIntLen <- hostIntrons[,c(8,7)]

flankingIntrons$intLength <- flankingIntrons$stop - flankingIntrons$start
flankingIntrons$group <- rep("flankingInt", length(flankingIntrons$id))
flankingIntLen <- flankingIntrons[,c(9,8)]



#Compare mean intron Length of all host RNA introns with circRNA flanking introns:

z.test(x=as.numeric(hostIntLen$intLength), sigma.x=sd(as.numeric(hostIntLen$intLength)), y=as.numeric(flankingIntLen$intLength), sigma.y=sd(as.numeric(flankingIntLen$intLength)))

hostVflnk_intlength_Data <- rbind(hostIntLen, flankingIntLen)
write.table(hostVflnk_intlength_Data, "hostVflnk_intlength.txt", quote = FALSE, row.names = FALSE)

#Density plots 
flnkLenComp <- rbind(hostIntLen, flankingIntLen)
flc <- ddply(flnkLenComp , "group", summarise, grp.med=median(intLength), grp.mean=mean(intLength), grp.se=std.error(intLength))
flc$group <- factor(flc$group, levels = c("hostInt", "flankingInt"))

densPlot_flc <- ggplot(flnkLenComp, aes(x=intLength, color=group, fill=group)) + geom_density(alpha=0.2, size = 0.5) + geom_vline(data=flc, aes(xintercept=grp.med, color=group), linetype="dashed", size=0.5) + scale_x_log10() + labs(x="Intron length (Log10(nts))", y="Density") + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "gray", size = 0.1),panel.grid.minor = element_blank(), panel.border = element_rect(fill=NA, colour = "darkgray", size = 0.5), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.text = element_text(family = "Helvetica", colour = "black", size = 7), axis.title = element_text(family = "Helvetica", colour = "black", size = 9), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("purple3","gray40")) + scale_fill_manual(values=c("purple3","gray40"))
densPlot_flc

#Bar plot of mean and standard error
barPlot_flc <- ggplot(flc, aes(x=group, y=grp.mean, color=group, fill=group)) + geom_bar(stat = "identity", alpha=0.2, size=0.3, width = 0.7) + labs(y="Intron length (nts)") + geom_errorbar(aes(ymin=grp.mean-grp.se, ymax=grp.mean+grp.se), width=0.4, size = 0.5) + theme(panel.background = element_blank(), panel.grid = element_blank(), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.line = element_line(colour = "darkgray", size = 0.5), axis.text.y = element_text(family = "Helvetica", colour = "black", size = 7), axis.text.x = element_text(family = "Helvetica", colour = "black", size = 11, angle = 45, vjust = 1, hjust = 1), axis.title.y = element_text(family = "Helvetica", colour = "black", size = 9, face = "bold"), axis.title.x = element_blank(), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "purple3")) + scale_fill_manual(values=c("gray40", "purple3")) + scale_y_continuous(expand=c(0,0,0.1,0)) 
barPlot_flc 


#Compare mean intron Length of all host RNA introns with non-circRNA flanking introns:

z.test(x=as.numeric(hostIntLen$intLength), sigma.x=sd(as.numeric(hostIntLen$intLength)), y=as.numeric(nonFlnkIntLen$intLength), sigma.y=sd(as.numeric(nonFlnkIntLen$intLength)))


hostVnonFlnk_intlength_Data <- rbind(hostIntLen, nonFlnkIntLen)
write.table(hostVnonFlnk_intlength_Data, "hostVnonFlnk_intlength.txt", quote = FALSE, row.names = FALSE)


#Density plots 
nonflnkLenComp <- rbind(hostIntLen, nonFlnkIntLen)
nflc <- ddply(nonflnkLenComp , "group", summarise, grp.med=median(intLength), grp.mean=mean(intLength), grp.se=std.error(intLength))
nflc$group <- factor(nflc$group, levels = c("hostInt", "nonFlnkInt"))

densPlot_nflc <- ggplot(nonflnkLenComp, aes(x=intLength, color=group, fill=group)) + geom_density(alpha=0.2, size = 0.5) + geom_vline(data=nflc, aes(xintercept=grp.med, color=group), linetype="dashed", size=0.5) + scale_x_log10() + labs(x="Intron length (Log10(nts))", y="Density") + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "gray", size = 0.1),panel.grid.minor = element_blank(), panel.border = element_rect(fill=NA, colour = "darkgray", size = 0.5), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.text = element_text(family = "Helvetica", colour = "black", size = 7), axis.title = element_text(family = "Helvetica", colour = "black", size = 9), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "orange")) + scale_fill_manual(values=c("gray40", "orange"))
densPlot_nflc

#Bar plot of mean and standard error
barPlot_nflc <- ggplot(nflc, aes(x=group, y=grp.mean, color=group, fill=group)) + geom_bar(stat = "identity", alpha=0.2, size=0.3, width = 0.7) + labs(y="Intron length (nts)") + geom_errorbar(aes(ymin=grp.mean-grp.se, ymax=grp.mean+grp.se), width=0.4, size = 0.5) + theme(panel.background = element_blank(), panel.grid = element_blank(), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.line = element_line(colour = "darkgray", size = 0.5), axis.text.y = element_text(family = "Helvetica", colour = "black", size = 7), axis.text.x = element_text(family = "Helvetica", colour = "black", size = 11, angle = 45, vjust = 1, hjust = 1), axis.title.y = element_text(family = "Helvetica", colour = "black", size = 9, face = "bold"), axis.title.x = element_blank(), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "orange")) + scale_fill_manual(values=c("gray40", "orange")) + scale_y_continuous(expand=c(0,0,0.1,0)) 
barPlot_nflc 

ggarrange(barPlot_flc, barPlot_nflc, common.legend = FALSE, legend = "right")
```
...
#Import bedtools results
  upIntrons_closestAlu <- read.delim("~/Documents/circRNAseq/ctrl9_diff/Intron_analysis/upIntrons_closestAlu.txt", header=FALSE)
...
dwnIntrons_closestAlu <- read.delim("~/Documents/circRNAseq/ctrl9_diff/Intron_analysis/dwnIntrons_closestAlu.txt", header=FALSE)
...
#calculate percentage of circs dwn at D28 by flanking intron IAE type
pIAE_short_dwnD28_per <- length(intersect(pIAE_short$circID, dwnD28vNES$id))/length(intersect(circsAnnotIntrons$id, dwnD28vNES$id))
pIAE_long_dwnD28_per <- length(intersect(pIAE_long$circID, dwnD28vNES$id))/length(intersect(circsAnnotIntrons$id, dwnD28vNES$id))
non_pIAE_short_dwnD28_per <- length(intersect(non_pIAE_short$id, dwnD28vNES$id))/length(intersect(circsAnnotIntrons$id, dwnD28vNES$id))
non_pIAE_long_dwnD28_per <- length(intersect(non_pIAE_long$id, dwnD28vNES$id))/length(intersect(circsAnnotIntrons$id, dwnD28vNES$id))




IAE_table <- as.data.frame(cbind(c(rep("allCircs", 4), rep("upD28circs", 4)), rep(c("pIAE-S", "dIAE-L", "pIAE-L", "dIAE-S"),2), c(pIAE_short_per, non_pIAE_long_per, pIAE_long_per, non_pIAE_short_per, pIAE_short_upD28_per, non_pIAE_long_upD28_per, pIAE_long_upD28_per, non_pIAE_short_upD28_per)))
colnames(IAE_table) = c("group", "subgroup", "value")
IAE_table$percent = as.numeric(IAE_table$value)*100
IAE_table$percent = as.numeric(IAE_table$percent)
IAE_table$group = factor(IAE_table$group)
IAE_barPlot <- ggplot(IAE_table, aes(x = group, y = percent, fill = subgroup, colour = subgroup)) +
  geom_bar(position = "stack", stat = "identity", width = 0.6, alpha = 0.7, colour = "white", size = 0.8) + 
  scale_y_continuous(expand = c(0,0,0.05,0)) + 
  labs(y="Flanking intron type (%)") +
  theme(panel.background = element_blank(), 
        panel.grid = element_blank(), 
        axis.ticks = element_line(colour = "darkgray", size = 0.5), 
        axis.line = element_line(colour = "darkgray", size = 0.5), 
        axis.text.y = element_text(family = "Helvetica", colour = "black", size = 7), 
        axis.text.x = element_text(family = "Helvetica", colour = "black", size = 11, angle = 45, vjust = 1, hjust = 1), 
        axis.title.y = element_text(family = "Helvetica", colour = "black", size = 9, face = "bold"), 
        axis.title.x = element_blank(), 
        legend.title = element_blank(), 
        legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) +
  scale_fill_manual(values=c("maroon4","maroon3","olivedrab4","olivedrab3"))

IAE_table2 <- as.data.frame(cbind(c(rep("allCircs", 4), rep("dwnD28circs", 4)), rep(c("pIAE-S", "dIAE-L", "pIAE-L", "dIAE-S"),2), c(pIAE_short_per, non_pIAE_long_per, pIAE_long_per, non_pIAE_short_per, pIAE_short_dwnD28_per, non_pIAE_long_dwnD28_per, pIAE_long_dwnD28_per, non_pIAE_short_dwnD28_per)))
colnames(IAE_table2) = c("group", "subgroup", "value")
IAE_table2$percent = as.numeric(IAE_table$value)*100
IAE_table2$percent = as.numeric(IAE_table$percent)
IAE_table2$group = factor(IAE_table2$group)
IAE_barPlot2 <- ggplot(IAE_table2, aes(x = group, y = percent, fill = subgroup, colour = subgroup)) +
  geom_bar(position = "stack", stat = "identity", width = 0.6, alpha = 0.7, colour = "white", size = 0.8) + 
  scale_y_continuous(expand = c(0,0,0.05,0)) + 
  labs(y="Flanking intron type (%)") +
  theme(panel.background = element_blank(), 
...
ggarrange(IAE_barPlot, IAE_barPlot2, common.legend = TRUE, legend = "right")
...
#Compare mean IAE distance of all circRNAs and circRNAs downregulated at D28
dwnD28_IAEcircs <- invertedAlu_circs[which(invertedAlu_circs$circID%in% dwnD28vNES$id),]
z.test(x=as.numeric(invertedAlu_circs$meanAluDist), sigma.x=sd(as.numeric(invertedAlu_circs$meanAluDist)), y=as.numeric(dwnD28_IAEcircs$meanAluDist), sigma.y=sd(as.numeric(dwnD28_IAEcircs$meanAluDist)))
...
#Compare mean intron Length of all circRNAs and circRNAs downregulated at D28
dwnD28_circsAnnotIntrons <- circsAnnotIntrons[which(circsAnnotIntrons$id %in% dwnD28vNES$id),]
z.test(x=as.numeric(circsAnnotIntrons$meanLengthIntrons), sigma.x=sd(as.numeric(circsAnnotIntrons$meanLengthIntrons)), y=as.numeric(dwnD28_circsAnnotIntrons$meanLengthIntrons), sigma.y=sd(as.numeric(dwnD28_circsAnnotIntrons$meanLengthIntrons)))


#Density plots 
circsAnnotIntrons$group <- rep("circs", length(circsAnnotIntrons$id))
upD28_circsAnnotIntrons$group <- rep("upD28_circs", length(upD28_circsAnnotIntrons$id))
totIntLength_circs <- circsAnnotIntrons[,c(28,27)]
totIntLength_upD28circs <- upD28_circsAnnotIntrons[,c(28,27)]

totIntLen <- rbind(totIntLength_circs, totIntLength_upD28circs)


mu <- ddply(totIntLen, "group", summarise, grp.mean=median(totIntLen))

densPlot_IntLen <- ggplot(totIntLen, aes(x=totIntLen, color=group, fill=group)) + geom_density(alpha=0.2, size = 0.5) + geom_vline(data=mu, aes(xintercept=grp.mean, color=group), linetype="dashed", size=0.5) + scale_x_log10() + labs(x="Total flanking intron length (Log10(nts))", y="Density") + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "gray", size = 0.1),panel.grid.minor = element_blank(), panel.border = element_rect(fill=NA, colour = "darkgray", size = 0.5), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.text = element_text(family = "Helvetica", colour = "black", size = 7), axis.title = element_text(family = "Helvetica", colour = "black", size = 9), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "purple3")) + scale_fill_manual(values=c("gray40", "purple3"))
densPlot_IntLen



invertedAlu_circs$group <- rep("circs", length(invertedAlu_circs$circID))
upD28_IAEcircs$group <- rep("upD28_circs", length(upD28_IAEcircs$circID))
dwnD28_IAEcircs$group <- rep("dwnD28_circs", length(dwnD28_IAEcircs$circID))
totIAEdist_circs <- invertedAlu_circs[,c(11,10)]
totIAEdist_upD28circs <- upD28_IAEcircs[,c(11,10)]
totIAEdist_dwnD28circs <- dwnD28_IAEcircs[,c(11,10)]

totIAEdist <- rbind(totIAEdist_circs, totIAEdist_upD28circs)
totIAEdist2 <- rbind(totIAEdist_circs, totIAEdist_dwnD28circs)

muIAE <- ddply(totIAEdist, "group", summarise, grp.mean=median(sumAluDist))
muIAE2 <- ddply(totIAEdist2, "group", summarise, grp.mean=median(sumAluDist))

densPlot_IAEdist <- ggplot(totIAEdist, aes(x=sumAluDist, color=group, fill=group)) + geom_density(alpha=0.2, size = 0.5) + geom_vline(data=muIAE, aes(xintercept=grp.mean, color=group), linetype="dashed", size=0.5) + scale_x_log10() + labs(x="Total distance to IAE (Log10(nts))", y="Density") + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "gray", size = 0.1),panel.grid.minor = element_blank(), panel.border = element_rect(fill=NA, colour = "darkgray", size = 0.5), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.text = element_text(family = "Helvetica", colour = "black", size = 7), axis.title = element_text(family = "Helvetica", colour = "black", size = 9), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "purple3")) + scale_fill_manual(values=c("gray40", "purple3"))
densPlot_IAEdist

densPlot_IAEdist2 <- ggplot(totIAEdist2, aes(x=sumAluDist, color=group, fill=group)) + geom_density(alpha=0.2, size = 0.5) + geom_vline(data=muIAE2, aes(xintercept=grp.mean, color=group), linetype="dashed", size=0.5) + scale_x_log10() + labs(x="Total distance to IAE (Log10(nts))", y="Density") + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "gray", size = 0.1),panel.grid.minor = element_blank(), panel.border = element_rect(fill=NA, colour = "darkgray", size = 0.5), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.text = element_text(family = "Helvetica", colour = "black", size = 7), axis.title = element_text(family = "Helvetica", colour = "black", size = 9), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "purple3")) + scale_fill_manual(values=c("gray40", "purple3"))
densPlot_IAEdist2


rm(totIAEdist_circs, totIAEdist_upD28circs, totIAEdist_dwnD28circs)
###
meanALUdist_circs <- invertedAlu_circs[,c(11,9)]
meanALUdist_upD28circs <- upD28_IAEcircs[,c(11,9)]
meanALUdist_dwnD28circs <- dwnD28_IAEcircs[,c(11,9)]
meanALUdist <- rbind(meanALUdist_circs, meanALUdist_upD28circs)
meanALUdist2 <- rbind(meanALUdist_circs, meanALUdist_dwnD28circs)

meanALUdist_sum <- ddply(meanALUdist, "group", summarise, grp.mean=mean(as.numeric(meanAluDist)), grp.se=std.err.mean(as.numeric(meanAluDist)))

meanALUdist_sum2 <- ddply(meanALUdist2, "group", summarise, grp.mean=mean(as.numeric(meanAluDist)), grp.se=std.err.mean(as.numeric(meanAluDist)))

rm(meanALUdist, meanALUdist_circs, meanALUdist_upD28circs, meanALUdist_dwnD28circs)

barPlot_ALUdist <- ggplot(meanALUdist_sum, aes(x=group, y=grp.mean, color=group, fill=group)) + geom_bar(stat = "identity", alpha=0.2, size=0.3, width = 0.7) + labs(y="Mean Alu distance (nts)") + geom_errorbar(aes(ymin=grp.mean-grp.se, ymax=grp.mean+grp.se), width=0.4, size = 0.5) + theme(panel.background = element_blank(), panel.grid = element_blank(), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.line = element_line(colour = "darkgray", size = 0.5), axis.text.y = element_text(family = "Helvetica", colour = "black", size = 7), axis.text.x = element_text(family = "Helvetica", colour = "black", size = 11, angle = 45, vjust = 1, hjust = 1), axis.title.y = element_text(family = "Helvetica", colour = "black", size = 9, face = "bold"), axis.title.x = element_blank(), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "purple3")) + scale_fill_manual(values=c("gray40", "purple3")) + scale_y_continuous(expand=c(0,0,0.1,0)) 
barPlot_ALUdist

barPlot_ALUdist2 <- ggplot(meanALUdist_sum2, aes(x=group, y=grp.mean, color=group, fill=group)) + geom_bar(stat = "identity", alpha=0.2, size=0.3, width = 0.7) + labs(y="Mean Alu distance (nts)") + geom_errorbar(aes(ymin=grp.mean-grp.se, ymax=grp.mean+grp.se), width=0.4, size = 0.5) + theme(panel.background = element_blank(), panel.grid = element_blank(), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.line = element_line(colour = "darkgray", size = 0.5), axis.text.y = element_text(family = "Helvetica", colour = "black", size = 7), axis.text.x = element_text(family = "Helvetica", colour = "black", size = 11, angle = 45, vjust = 1, hjust = 1), axis.title.y = element_text(family = "Helvetica", colour = "black", size = 9, face = "bold"), axis.title.x = element_blank(), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "purple3")) + scale_fill_manual(values=c("gray40", "purple3")) + scale_y_continuous(expand=c(0,0,0.1,0)) 
barPlot_ALUdist2 


#Bar plot of mean and standard error
meanIntLen_circs <- circsAnnotIntrons[,c(28,24)]
meanIntLen_upD28circs <- upD28_circsAnnotIntrons[,c(28,24)]
meanIntLen <- rbind(meanIntLen_circs, meanIntLen_upD28circs)

meanIntLen_sum <- ddply(meanIntLen, "group", summarise, grp.mean=mean(as.numeric(meanLengthIntrons)), grp.se=std.err.mean(as.numeric(meanLengthIntrons)))

barPlot_IntLen <- ggplot(meanIntLen_sum, aes(x=group, y=grp.mean, color=group, fill=group)) + geom_bar(stat = "identity", alpha=0.2, size=0.3, width = 0.7) + labs(y="Mean intron length (nts)") + geom_errorbar(aes(ymin=grp.mean-grp.se, ymax=grp.mean+grp.se), width=0.4, size = 0.5) + theme(panel.background = element_blank(), panel.grid = element_blank(), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.line = element_line(colour = "darkgray", size = 0.5), axis.text.y = element_text(family = "Helvetica", colour = "black", size = 7), axis.text.x = element_text(family = "Helvetica", colour = "black", size = 11, angle = 45, vjust = 1, hjust = 1), axis.title.y = element_text(family = "Helvetica", colour = "black", size = 9, face = "bold"), axis.title.x = element_blank(), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "purple3")) + scale_fill_manual(values=c("gray40", "purple3")) + scale_y_continuous(expand=c(0,0,0.1,0)) 

#Compare mean intron Length of all circRNAs and circRNAs downregulated at D28
dwnD28_circsAnnotIntrons <- circsAnnotIntrons[which(circsAnnotIntrons$id %in% dwnD28vNES$id),]
dwnD28_circsAnnotIntrons$group <- rep("dwnD28_circs", length(dwnD28_circsAnnotIntrons$id))

z.test(x=as.numeric(circsAnnotIntrons$meanLengthIntrons), sigma.x=sd(as.numeric(circsAnnotIntrons$meanLengthIntrons)), y=as.numeric(dwnD28_circsAnnotIntrons$meanLengthIntrons), sigma.y=sd(as.numeric(dwnD28_circsAnnotIntrons$meanLengthIntrons)))

totIntLength_dwnD28circs <- dwnD28_circsAnnotIntrons[,c(28,27)]
totIntLen2 <- rbind(totIntLength_circs, totIntLength_dwnD28circs)
rm(totIntLength_circs, totIntLength_upD28circs, totIntLength_dwnD28circs)

mu2 <- ddply(totIntLen2, "group", summarise, grp.mean=median(totIntLen))

densPlot_IntLen2 <- ggplot(totIntLen2, aes(x=totIntLen, color=group, fill=group)) + geom_density(alpha=0.2, size = 0.5) + geom_vline(data=mu2, aes(xintercept=grp.mean, color=group), linetype="dashed", size=0.5) + scale_x_log10() + labs(x="Total flanking intron length (Log10(nts))", y="Density") + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "gray", size = 0.1),panel.grid.minor = element_blank(), panel.border = element_rect(fill=NA, colour = "darkgray", size = 0.5), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.text = element_text(family = "Helvetica", colour = "black", size = 7), axis.title = element_text(family = "Helvetica", colour = "black", size = 9), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "purple3")) + scale_fill_manual(values=c("gray40", "purple3"))
densPlot_IntLen2

#Bar plot of mean and standard error
meanIntLen_circs <- circsAnnotIntrons[,c(28,24)]
meanIntLen_dwnD28circs <- dwnD28_circsAnnotIntrons[,c(28,24)]
meanIntLen2 <- rbind(meanIntLen_circs, meanIntLen_dwnD28circs)

meanIntLen_sum2 <- ddply(meanIntLen2, "group", summarise, grp.mean=mean(as.numeric(meanLengthIntrons)), grp.se=std.err.mean(as.numeric(meanLengthIntrons)))
rm(meanIntLen, meanIntLen2, meanIntLen_circs, meanIntLen_upD28circs, meanIntLen_dwnD28circs)

barPlot_IntLen2 <- ggplot(meanIntLen_sum2, aes(x=group, y=grp.mean, color=group, fill=group)) + geom_bar(stat = "identity", alpha=0.2, size=0.3, width = 0.7) + labs(y="Mean intron length (nts)") + geom_errorbar(aes(ymin=grp.mean-grp.se, ymax=grp.mean+grp.se), width=0.4, size = 0.5) + theme(panel.background = element_blank(), panel.grid = element_blank(), axis.ticks = element_line(colour = "darkgray", size = 0.5), axis.line = element_line(colour = "darkgray", size = 0.5), axis.text.y = element_text(family = "Helvetica", colour = "black", size = 7), axis.text.x = element_text(family = "Helvetica", colour = "black", size = 11, angle = 45, vjust = 1, hjust = 1), axis.title.y = element_text(family = "Helvetica", colour = "black", size = 9, face = "bold"), axis.title.x = element_blank(), legend.title = element_blank(), legend.text = element_text(family = "Helvetica", colour = "black", size = 8)) + scale_colour_manual(values=c("gray40", "purple3")) + scale_fill_manual(values=c("gray40", "purple3")) + scale_y_continuous(expand=c(0,0,0.1,0)) 
barPlot_IntLen2

ggarrange(barPlot_IntLen2, barPlot_ALUdist2, common.legend = TRUE, legend = "right")
